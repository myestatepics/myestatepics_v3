...
def classify_tones(rgb: np.ndarray, scene: Scene, settings: dict | None = None) -> tuple[dict, dict]:
    ...
    # v3.2 Fix 1: neutrality is a COLOR question (chroma flatness), not a brightness
    # question. A dark neutral ceiling is still neutral -- darkness is exposure's
    # job. Only a low sanity floor remains so we never trust near-black noise.
