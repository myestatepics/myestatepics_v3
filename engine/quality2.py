...
def evaluate(
    ...
    *,
    ...
    if wall_target is not None and np.any(wall_mask):
    ...
    if np.any(ceiling_mask) and ceiling_tone:
    ...
    src_lab = cv2.cvtColor(original, cv2.COLOR_RGB2LAB).astype(np.float32)
    ...
    material_hue_p95, material_saturation_p95 = _hsv_drift(original, final, np.maximum(protected_f, floor_mask_f))
